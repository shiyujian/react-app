...
//初始化区域图的对象
	...
	this.$nodeRemark= property.remark || {};//每一种结点或按钮的说明文字,JSON格式,key为类名,value为用户自定义文字说明
	...
	if(property.haveHead){
      ...
      tmp+="<label title='"+(property.initLabelText||"newFlow_1")+"' "
        ...
        this.onEditClick=null;// 启动编辑按钮定义事件
        this.onFullsreenClick=null; //启动全屏编辑
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
                ...
                case "ico_edit": if(This.onEditClick!=null) This.onEditClick();break;
                case "ico_zoomin": This.zoomin();break;
                case "ico_zoomout": This.zoomout(); break;
                case "ico_fullsreen": if(This.onFullsreenClick!=null)	This.onFullsreenClick();break;
			}
		});
	}
	var toolWidth=0;
	if(property.haveTool){
		...
		if(property.toolBtns&&property.toolBtns.length>0){
			...
			for(var i=0;i<property.toolBtns.length;++i){
				for(var key in this.$nodeRemark) {
					if(property.toolBtns[i].indexOf(key) > -1) {
						name = this.$nodeRemark[key];
					}
				}

				tmp+="<a href='javascript:void(0)' type='"+property.toolBtns[i]+"' id='"+this.$id+"_btn_"+property.toolBtns[i].split(" ")[0]+"' class='GooFlow_tool_btn'><i class='ico_"+property.toolBtns[i]+"'/><p>"+ name +"</p></a>";//加入自定义按钮
			}
...
GooFlow.prototype={
	...
	focusItem:function(id,bool){
		...
		this.$focus=id;
		this.$editable && this.switchToolBtn("cursor");
	},
	//移动结点到一个新的位置
